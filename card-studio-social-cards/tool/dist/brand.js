...
exports.DEFAULT_BRAND = {
    name: 'ClawMama',
    domain: 'clawmama.run',
    markSvg: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48">' +
...
function resolveBrand(partial) {
    if (!partial)
        return exports.DEFAULT_BRAND;
    // A renamed brand must not inherit the ClawMama mark: without an explicit
    // markSvg it renders wordmark-only.
    const renamed = partial.name !== undefined && partial.name !== exports.DEFAULT_BRAND.name;
    const markSvg = partial.markSvg !== undefined ? partial.markSvg : renamed ? undefined : exports.DEFAULT_BRAND.markSvg;
    return {
        name: partial.name ?? exports.DEFAULT_BRAND.name,
        domain: partial.domain ?? exports.DEFAULT_BRAND.domain,
        ...(markSvg ? { markSvg } : {}),
    };
}
/**
 ...
 */
function markDataUri(brand, theme, primary) {
    if (!brand.markSvg)
        return null;
