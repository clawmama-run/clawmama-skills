...
exports.isZh = isZh;
exports.sceneOf = sceneOf;
exports.weightedLen = weightedLen;
exports.titleSize = titleSize;
exports.displayFamily = displayFamily;
exports.sansFamily = sansFamily;
exports.eyebrowPill = eyebrowPill;
exports.footerRow = footerRow;
exports.brandMark = brandMark;
exports.brandRow = brandRow;
exports.orbitalArcs = orbitalArcs;
exports.canvas = canvas;
const h_1 = require("../h");
const brand_1 = require("../brand");
exports.CJK_RE = /[　-鿿豈-﫿]/;
function isZh(c) {
    return c.lang === 'zh' || exports.CJK_RE.test([c.title, c.quote, c.subtitle, c.body].filter(Boolean).join(''));
}
function sceneOf(c) {
    if (c.quote)
        return 'quote';
 ...
 */
function titleSize(ctx, title, base) {
    const byAspect = { landscape: 74, square: 84, portrait: 88, tall: 92 };
...
function displayFamily(ctx) {
    if (ctx.zh && ctx.t.fontDisplayCjk)
        return ctx.t.fontDisplayCjk;
    if (ctx.zh)
        return 'Noto Sans SC';
    return ctx.t.fontDisplay;
}
function sansFamily(ctx) {
    return ctx.zh ? 'Noto Sans SC' : ctx.t.fontSans;
}
/** Small uppercase label pill with a dot, used as the eyebrow. */
function eyebrowPill(ctx, label) {
    const { t, u } = ctx;
    return (0, h_1.h)('div', {
        alignItems: 'center',
        gap: u(10),
    ...
    }, (0, h_1.h)('div', { width: u(9), height: u(9), borderRadius: 999, backgroundColor: t.brand }), (0, h_1.text)(label, {
...
function footerRow(ctx, opts = {}) {
    const { c, t, u } = ctx;
    const color = opts.color ?? t.brand;
    const muted = opts.mutedColor ?? t.muted;
    const right = [];
    if (c.badge) {
        right.push((0, h_1.h)('div', {
            border: `1px solid ${t.surfaceBorder}`,
            borderRadius: 999,
            padding: `${u(6)}px ${u(14)}px`,
        }, (0, h_1.text)(c.badge, { fontFamily: t.fontMono, fontSize: u(19), fontWeight: 700, color: muted, letterSpacing: u(1) })));
    }
    if (c.page) {
        right.push((0, h_1.text)(`${String(c.page.index).padStart(2, '0')} / ${String(c.page.count).padStart(2, '0')}`, {
    ...
    return (0, h_1.h)('div', { justifyContent: 'space-between', alignItems: 'center', width: '100%' }, (0, h_1.text)(c.footer ?? ctx.b.domain, {
        fontFamily: t.fontMono,
        fontSize: u(21),
        fontWeight: 700,
        color,
        letterSpacing: u(1),
    }), (0, h_1.h)('div', { alignItems: 'center', gap: u(12) }, ...right));
}
/** Logo mark: the brand's real SVG, tinted by the theme, as an <img>. */
function brandMark(ctx, size, color) {
    const src = (0, brand_1.markDataUri)(ctx.b, ctx.t, color);
    if (!src)
        return null;
    return { type: 'img', props: { src, width: size, height: size, style: { width: size, height: size } } };
}
/** Brand mark + wordmark row. */
function brandRow(ctx, opts = {}) {
    const { t, u } = ctx;
    const size = opts.size ?? u(44);
    return (0, h_1.h)('div', { alignItems: 'center', gap: u(14) }, brandMark(ctx, size, opts.color), (0, h_1.text)(ctx.b.name, {
...
function orbitalArcs(ctx, opts = {}) {
    const { u, t } = ctx;
    const size = opts.size ?? u(420);
    const color = opts.color ?? t.brand;
    const ring = (scale, alpha, rot, strokeMul) => (0, h_1.h)('div', {
        position: 'absolute',
    ...
    }, ring(1.0, '52', 28, 1), ring(0.78, '7a', -12, 1.1), ring(0.56, 'a8', 52, 1.2), (0, h_1.h)('div', {
        position: 'absolute',
        right: size * 0.3,
        top: size * 0.42,
        width: size * 0.11,
...
function canvas(ctx, style, ...children) {
    const { t, w, h: hh } = ctx;
